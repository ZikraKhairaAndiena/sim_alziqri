                    ...
                    <div class="card-body">
                        ...
                        @if (auth()->user()->role === 'orang_tua')
                            ...
                            @php
                                ...
                                $persentaseBayar =
                                    $totalTagihan > 0 ? round(($totalTerbayar / $totalTagihan) * 100, 2) : 0;
                            @endphp
                            ...
                            <div class="alert alert-info">
                                ...
                                <p class="mb-1 text-success">Sudah Dibayar: Rp
                                    {{ number_format($totalTerbayar, 0, ',', '.') }}</p>
                                <p class="mb-1 text-danger">Sisa Tagihan: Rp {{ number_format($sisaTagihan, 0, ',', '.') }}
                                </p>

                                <div class="progress" style="height: 20px;">
                                    <div class="progress-bar bg-success" role="progressbar"
                                        style="width: {{ $persentaseBayar }}%" aria-valuenow="{{ $persentaseBayar }}"
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        @foreach ($pembayarans as $pembayaran)
                                            <tr>
                                                <td>{{ \Carbon\Carbon::parse($pembayaran->tgl_bayar)->format('d-m-Y') }}
                                                </td>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('admin.pembayaran.detail', $siswa->id) }}"
                                                        class="btn btn-info btn-sm me-1" title="Lihat Detail"><i
                                    ...
                                    <nav aria-label="Page navigation">
                                        <ul class="pagination">
                                            ...
                                            @for ($i = 1; $i <= $siswaList->lastPage(); $i++)
                                                <li
                                                    class="page-item {{ $siswaList->currentPage() == $i ? 'active' : '' }}">
                                                    <a class="page-link"
                                                        href="{{ $siswaList->url($i) }}">{{ $i }}</a>
                                                </li>
                                            @endfor
