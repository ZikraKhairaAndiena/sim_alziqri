        ...
        <form action="{{ route('admin.thn_ajaran.update', $thn_ajaran->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('nama') is-invalid @enderror"
                       id="nama" name="nama" value="{{ old('nama', $thn_ajaran->nama) }}" placeholder="Contoh: 2025/2026" maxlength="15" required>
                @error('nama')
            ...
            <div class="mb-3">
                ...
                <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                    ...
                    <option value="aktif" {{ old('status', $thn_ajaran->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                    <option value="tidak_aktif" {{ old('status', $thn_ajaran->status) == 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
