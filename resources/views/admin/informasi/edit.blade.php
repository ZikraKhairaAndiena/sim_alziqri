            ...
            <div class="card-body">
                <form action="{{ route('admin.informasi.update', $informasi->id) }}" method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="content" rows="5" class="form-control @error('content') is-invalid @enderror" required>{{ old('content', $informasi->content) }}</textarea>
                        @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                            <option value="pengumuman"
                                {{ old('type', $informasi->type) == 'pengumuman' ? 'selected' : '' }}>Pengumuman</option>
                            <option value="info" {{ old('type', $informasi->type) == 'info' ? 'selected' : '' }}>Info
                            </option>
                            <option value="galeri" {{ old('type', $informasi->type) == 'galeri' ? 'selected' : '' }}>Galeri
                            </option>
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="mb-3">
                        ...
                        @if ($informasi->gambar)
                            <img src="{{ asset('img/' . $informasi->gambar) }}" alt="Foto informasi" width="120"
                                class="mb-2"><br>
                        @endif
